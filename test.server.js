...
(function() {
  var app, fs, nconf, password, put, putWithoutAuth, request, server, settings, should, testData, user;
  ...
  put = function(entry, expectStatus, assertCallback) {
    return request(app).put('/entry').type('application/json').auth(user, password).send(entry).expect('Content-Type', /json/).end(assertCallback);
  };
